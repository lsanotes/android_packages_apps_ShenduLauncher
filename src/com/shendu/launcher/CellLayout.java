...
public class CellLayout extends ViewGroup {
    ...
    public boolean mIsCurrentPage = false; //for drag crosshairs
    ...
    static int widthInPortrait(Resources r, int numCells) {
        ...
        int cellWidth = r.getDimensionPixelSize(R.dimen.workspace_cell_width_port);
        int minGap = Math.min(r.getDimensionPixelSize(R.dimen.workspace_width_gap_port),
                r.getDimensionPixelSize(R.dimen.workspace_height_gap_port));
    ...
    static int heightInLandscape(Resources r, int numCells) {
        ...
        // have. We ignore the left/right padding on CellLayout because it turns out in our design
        // the padding extends outside the visible screen size, but it looked fine anyway.
        int cellHeight = r.getDimensionPixelSize(R.dimen.workspace_cell_height_port);
        int minGap = Math.min(r.getDimensionPixelSize(R.dimen.workspace_width_gap_port),
                r.getDimensionPixelSize(R.dimen.workspace_height_gap_port));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mBackgroundAlpha > 0.0f && (existsLastOccupiedCell()[0]!=-1)) { //draw not empty celllayout background
            Drawable bg;
        	//Log.i(Launcher.TAG, TAG+"==onDraw()=mBackgroundAlpha=="+mBackgroundAlpha+
        			//"==mBackgroundAlphaMultiplier=="+mBackgroundAlphaMultiplier);
    ...
    public void changedCellInfoStatus(){//add for remove empty screen
    	onAttachedToWindow() ;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (!LauncherApplication.isScreenLarge()){
        	if(mCountX!=0 && mCountY!=0){ //null point exception
    ...
    public void setFastBackgroundAlpha(float alpha) {//for effect
    ...
    */
    ...
    public static int[] rectToCell(Resources resources, int width, int height, int[] result) {
        ...
        int actualWidth = resources.getDimensionPixelSize(R.dimen.workspace_cell_width_port);
        int actualHeight = resources.getDimensionPixelSize(R.dimen.workspace_cell_height_port);
